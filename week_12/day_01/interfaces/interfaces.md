...
In this lesson we are going to concentrate on the opposite. That is, we are going to see create our own interfaces, so that objects have to conform to our contracts.
...
On the other hand, if we go with option (2) then *EVERY* class which implements `IRun`, like our `Bicycle` would now also need to have a `start()` method. One thing about using inheritance or interfaces is that we shouldn't force methods onto classes that they don't need.
...
2. Interface combining multiple other interfaces
	...
	> in IntelliJ Create a new Java interface called `ICaDrive` and write:
