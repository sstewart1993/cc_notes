...
2. When the form is submitted, `handleSubmit` creates an object using the submitted values. It then carries out a `POST` request, passing the object as a second optional argument, that specifies three properties:
- the method - POST
- the body - the object created from the submitted values
- the data type being sent - JSON

3. A `fetch` request is sent. with the body sent as JSON (just as in Insomnia), the method set to `POST`, and the `Content-Type` set to "application/json". Each of these steps is necessary.

4. When the server responds, we use the `eventBus` to refresh the data and then we clear the form.
