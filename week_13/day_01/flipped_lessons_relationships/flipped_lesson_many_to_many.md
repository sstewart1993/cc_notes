...
We need to add an annotation at the top of any test that interacts with the database so it knows to search for the config properties.

At the top of a test class that you're using add the `@AutoConfigureTestDatabase` annotation. For example:

```java
@AutoConfigureTestDatabase       //NEW
@SpringBootTest
